...
df <-
  map(
    dir_ls(here::here("logs"), glob = "*.json"),
    get_scores
  ) |>
  list_rbind() |>
  mutate(
...
df |>
  ...
  labs(
    ...
    fill = "Score",
    subtitle = "Performance is broadly similar across models, with Gemini 2.5 Pro and newer\nOpenAI models slightly ahead."
  ) +
  theme_light() +
  theme(
...
eval_summary |>
  ...
  labs(
    ...
    subtitle = "Cost is based on the total tokens used across the 291 Pandas problems\nin the evaluation."
  ) +
  theme_light() +
  theme(
    plot.subtitle = element_text(face = "italic")
