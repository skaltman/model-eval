...
**For Pandas coding tasks, we recommend using OpenAI's o4-mini or o3-mini or Anthropic's Claude Sonnet 4.** The top models all performed very similarly, but their prices vary substantially. **o4-mini, o3-mini, and Claude Sonnet 4 are substantially less expensive than Gemini 2.5 Pro, o1, and o3** (see Pricing Table below). For Pandas code generation, we expect that the additional cost will not translate to proportionally better results. 
...
LLM pricing is typically provided per million tokens. Note that in our analysis, Gemini 2.5 Pro, o1, o3, o3-mini, and o4-mini performed similarly for Python code generation, but Gemini 2.5 Pro, o1, and o3 are much more expensive. 
...
In our evaluation process, each model used between `r format(eval_tokens_summary$min_input, big.mark = ",")` and `r format(eval_tokens_summary$max_input, big.mark = ",")` input tokens and between `r format(eval_tokens_summary$min_output, big.mark = ",")` and `r format(eval_tokens_summary$max_output, big.mark = ",")` output tokens (including reasoning tokens). 
